...
/* ----------------------------------------------------------------------------
 * Function      : void GAPM_StartScanCmd(const struct gapm_start_advertise_cmd
 *                                        *startScanCmd)
 * ----------------------------------------------------------------------------
 * Description   : Send a GAPM_START_SCAN_CMD to the stack to start scanning for
 *                 peer devices
 * Inputs        : startScanCmd     - Scanning parameters
 * Outputs       : None
 * Assumptions   : None
 * ------------------------------------------------------------------------- */
void GAPM_StartScanCmd(const struct gapm_start_scan_cmd *startScanCmd)
{
    struct gapm_start_scan_cmd *cmd;
    gap_env.gapmState = GAPM_STATE_SCANNING;

    /* Create the scan command */
    cmd = KE_MSG_ALLOC(GAPM_START_SCAN_CMD, TASK_GAPM, TASK_APP,
                       gapm_start_scan_cmd);

    memcpy(cmd, startScanCmd, sizeof(struct gapm_start_scan_cmd));
                        ...
                        uint8_t irkListSize, const struct gap_sec_key *irkList)
{
    ...
    for (uint8_t i = 0;i < GAP_BD_ADDR_LEN; i++)
        PRINTF("%d ", cmd->addr.addr[i]);

    for(uint8_t j = 0; j < cmd->nb_key; j++)
    {
        PRINTF("\r\n  cmd->IRK[%d]: ", j);
        for (uint8_t i = 0;i < GAP_KEY_LEN; i++)
            PRINTF("%d ", cmd->irk[j].key[i]);
    }
...
/* ----------------------------------------------------------------------------
 * Function      : void GAPM_LepsmRegisterCmd(uint16_t le_psm,
 *                                            uint16_t app_task,
 *                                            uint8_t sec_lvl)
 * ----------------------------------------------------------------------------
 * Description   : Register a LE Protocol/Service Multiplexer command
 * Inputs        : - le_psm   - LE Protocol/Service Multiplexer
 *                 - app_task - Application task number
 *                 - sec_lvl  - Security level
 *                                7   6   5   4   3   2   1   0
 *                              +---+---+---+---+---+---+---+---+
 *                              |MI |      RFU      |EKS|SEC_LVL|
 *                              +---+---+---+---+---+---+---+---+
 *                              bit[0-1]: Security level requirement
 *                                        (0=NO_AUTH, 1=UNAUTH, 2=AUTH,
 *                                         3=SEC_CON)
 *                              bit[2]  : Encryption Key Size length must have
 *                                        16 bytes
 *                              bit[7]  : Is the application task
 *                                        multi-instantiated or not
 * Outputs       : None
 * Assumptions   : None
 * ------------------------------------------------------------------------- */
void GAPM_LepsmRegisterCmd(uint16_t le_psm, uint16_t app_task, uint8_t sec_lvl)
{
    struct gapm_lepsm_register_cmd* coc_cmd = KE_MSG_ALLOC(GAPM_LEPSM_REGISTER_CMD,
                                                           TASK_GAPM, TASK_APP,
                     ...
                     ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    switch(msg_id)
    {
        case GAPM_CMP_EVT:
        {
            ...
            PRINTF("\r\nGAPM_CMP_EVT: operation=0x%02x, status=0x%02x\r\n", p->operation, p->status);
...
/* ----------------------------------------------------------------------------
 * Function      : void GAPC_ParamUpdateCmd(uint8_t conidx, uint16_t intv_min,
 *                      uint16_t intv_max, uint16_t latency, uint16_t time_out,
 *                      uint16_t ce_len_min, uint16_t ce_len_max)
 * ----------------------------------------------------------------------------
 * Description   : Send parameter update request
 * Inputs        : - conidx       - Connection index
 *                 - intv_min     - Connection interval minimum
 *                 - intv_max     - Connection interval maximum
 *                 - latency      - Latency
 *                 - time_out     - Supervision timeout
...
/* ----------------------------------------------------------------------------
 * Function      : void GAPC_ConnectionCfm(uint8_t conidx, struct
 *                                                  gapc_connection_cfm *param)
 * ----------------------------------------------------------------------------
                        ...
                        const union gapc_dev_info_val* dat)
{
    ...
    {
        sz = offsetof(struct gapc_get_dev_info_cfm, info.name.value)+dat->name.length;
    }
    else
    {
    ...
    cfm = (struct gapc_get_dev_info_cfm*) ke_msg_alloc(
						...
						size_t name_max_length)
{
    ...
    cfm = (struct gapc_set_dev_info_cfm*) KE_MSG_ALLOC(GAPC_SET_DEV_INFO_CFM,
    			KE_BUILD_ID(TASK_GAPC, conidx),
				TASK_APP,
                    ...
                    ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    switch(msg_id)
    {
        ...
        case GAPC_CMP_EVT:
        {
            ...
            PRINTF("    \r\nGAPC_CMP_EVT: op=%d status=%d\r\n", ((struct gapc_cmp_evt*)param)->operation,
        ...
        case GAPC_PARAM_UPDATED_IND:
        {
            ...
            PRINTF("\r\nGAPC_PARAM_UPDATED_IND: con_interval=%d, con_latency = %d, sup_to = %d\r\n",
                    p->con_interval,
                    p->con_latency,
                    p->sup_to);
        ...
        case GAPC_BOND_IND:
        {
            ...
            switch(p->info)
            {
                ...
                {
                    PRINTF("\r\nGAPC_BOND_IND / GAPC_PAIRING_SUCCEED");
                ...
                case GAPC_IRK_EXCH:
                {
                    PRINTF("\r\nGAPC_BOND_IND / GAPC_IRK_EXCH:");
                    PRINTF("\r\n  IRK: ");
                    for (uint8_t i = 0;i < GAP_KEY_LEN; i++)
                        PRINTF("%d ",p->data.irk.irk.key[i]);

                    PRINTF("\r\n  ADDR_TYPE: %d", p->data.irk.addr.addr_type);

                    PRINTF("\r\n  ADDR:");
                    for (uint8_t i = 0;i < GAP_BD_ADDR_LEN; i++)
                        PRINTF("%d ", p->data.irk.addr.addr.addr[i]);
                ...
                {
                    PRINTF("\r\nGAPC_PAIRING_FAILED! Reason = %d", p->data.reason);
                }
                break;
            }
        }
